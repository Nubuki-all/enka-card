...
def generate_image(
    ...
    for index, constellation in enumerate(character.constellations):
        ...
        constellation_icon = open_image(
            path=f"{dir}/attributes/Genshin/UI/{constellation.icon.filename}.png",
    ...
    """ Talents Section """
    talent_overlay = open_image(f"{dir}/attributes/Assets/enka_talent_overlay.png")
    ...
    for index, skill in enumerate(character.skills):
        ...
        sk = open_image(
            path=f"{dir}/attributes/Genshin/UI/{skill.icon.filename}.png",
    ...
    weapon_image = open_image(
        path=f"{dir}/attributes/Genshin/Weapon/{weapon.detail.icon.filename}.png",
    ...
    rarity_light = scale_image(
        open_image(
            f"{dir}/attributes/UI/{RARITY_REFERENCE[str(weapon.detail.rarity)]}_WEAPON_LIGHT.png"
        ),
        fixed_height=40,
    )
    ...
    rarity = scale_image(
        open_image(f"{dir}/attributes/UI/{RARITY_REFERENCE[str(weapon.detail.rarity)]}.png"),
        fixed_height=25,
    )
    ...
    def draw_weapon_information(line_buffer: int = 0):
        ...
        image = open_image(f"{dir}/attributes/UI/{get_stat_filename(mainstat.prop_id)}.png")
        ...
        if substat:
            ...
            image = open_image(f"{dir}/attributes/UI/{get_stat_filename(substat.prop_id)}.png")
            icon_file = scale_image(image, fixed_height=30)
            icon_file = ImageEnhance.Brightness(icon_file).enhance(2)
    ...
    for index, item in enumerate(all_stats):
        """Draw Icon for Stat"""
        image = open_image(f"{dir}/attributes/UI/{get_stat_filename(item)}.png")
        icon_file = scale_image(image, fixed_height=30)
        icon_file = ImageEnhance.Brightness(icon_file).enhance(2)
    ...
    for artif_index, equipment_type in enumerate(positions):
        ...
        artif_icon = fade_asset_icon(
            open_image(
                path=f"{dir}/attributes/Genshin/Artifact/{artifact.detail.icon.filename}.png",
        ...
        image = open_image(
            f"{dir}/attributes/UI/{get_stat_filename(artifact.detail.mainstats.prop_id)}.png"
        )
        icon_file = scale_image(image, fixed_height=30)
        icon_file = ImageEnhance.Brightness(icon_file).enhance(2)
        ...
        rarity = scale_image(
            open_image(
                f"{dir}/attributes/UI/{RARITY_REFERENCE[str(artifact.detail.rarity)]}.png"
            ),
            fixed_height=18,
        )
        ...
        for index, subst in enumerate(artifact.detail.substats):
            ...
            image = open_image(f"{dir}/attributes/UI/{get_stat_filename(subst.prop_id)}.png")
            icon_file = scale_image(image, fixed_height=30)
            icon_file = ImageEnhance.Brightness(icon_file).enhance(2)
